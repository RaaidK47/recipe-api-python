...
class PrivateTagsApiTests(TestCase):   
    ...
    def test_retrieve_tags(self):
        ...
        tags = Tag.objects.all().order_by('-name')  # Order by Reverse Name Order
        # ^Making Query to DB

        serializer = TagSerializer(tags, many=True)  # Serialize result from Query
    ...
    def test_tags_limited_to_user(self):
        ...
        Tag.objects.create(user=user2, name='Fruity')  # Create a Tag for Non Authenticated User

        tag = Tag.objects.create(user=self.user, name='Comfort Food')  # Create a Tag for Authenticated User
        res = self.client.get(TAGS_URL)  # Retreiving Tags
    ...
    def test_filtered_tags_unique(self):
        ...
        # NOTE: `assigned_only` is a Query Parameter that is used to filter the Tags that are assigned to Recipes."""
